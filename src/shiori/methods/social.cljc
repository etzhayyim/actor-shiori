(ns shiori.methods.social
  "social.cljc — shiori DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from shiori's own
...
(defn draft-observation-post
  "A dry-run observation post over shiori's own domain (Human-Wellbecoming detractor observatory + transparent intervention — weaves the STRUCTURAL factors that diminish Wellbecoming…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "shiori R0: live social posting is Council Lv6+ + operator + "
