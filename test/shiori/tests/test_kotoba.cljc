...
(defn- tmp [] (str (System/getProperty "java.io.tmpdir") "/shiori-ledger-test-" (gensym) ".edn"))
(defn- d1 [] [(k/add "cohort:gig-workers" ":organism/kind" ":cohort")
              (k/add "cohort:gig-workers" ":cohort/scope" ":aggregate")])
(defn- d2 [] [(k/add "en.detractor:precarity.detracts.cohort:gig-workers" ":en/kind" ":detracts")
              (k/add "en.detractor:precarity.detracts.cohort:gig-workers" ":en/load" 0.75)])
...
(deftest float-load-roundtrips
  (let [p (tmp)]
    (try
      (k/append-tx (k/make-tx (d2) "t1" "as1" "") p)
      (is (= (d2) (get (first (k/read-log p)) ":tx/datoms")) "float :en/load round-trips")
      (finally (io/delete-file p true)))))
  ...
  (let [p (tmp)]
    (try
      (let [c1 (k/append-tx (k/make-tx (d1) "t1" "as1" "") p)]
        ...
        (let [corrupted (str/replace (slurp p) ":detracts" ":relieves")]
          (spit p corrupted)
...
;; ── heartbeat (autorun) over the REAL committed wellbecoming seed ────────────

(deftest beat-ground-only-and-real-seed
  (let [ds (auto/ground-datoms)]
    (is (seq ds) "ground datoms are produced from the real committed wellbecoming seed")
    (is (every? #(= ":db/add" (first %)) ds) "every datom is an :add (EAVT op)")
    (is (some (fn [[_ _ a]] (= ":en/from" a)) ds) "detraction/relief 縁 are persisted (graph not just nodes)")
    (is (not-any? (fn [[_ _ a]] (str/starts-with? (str a) ":bond/")) ds)
        "GROUND only — derived :bond/* readouts not persisted (N1/G2)")))

(deftest beat-no-per-person-affect
  ;; G1: a RELIEF map at COHORT-AGGREGATE scale — NEVER per-person affect/manipulation
  (let [ds (auto/ground-datoms)]
    (is (not-any? (fn [[_ _ a]] (let [s (str a)]
                                  (or (str/includes? s "person") (str/includes? s "individual")
                                      (str/includes? s "affect") (str/includes? s "sentiment")
                                      (str/includes? s "manipul")))) ds)
        "no per-person affect/manipulation datoms in the ledger (G1, cohort-aggregate only)")))
...
#?(:clj
   (when (= *file* (System/getProperty "babashka.file"))
     (let [{:keys [fail error]} (run-tests 'shiori.tests.test-kotoba)]
