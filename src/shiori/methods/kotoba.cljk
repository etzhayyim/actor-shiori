#!/usr/bin/env bb
;; shiori 栞 — human-Wellbecoming graph DATOM-LOG writer (content-addressed commit-DAG).
(ns shiori.methods.kotoba
  "kotoba.cljc — shiori 栞 append-only WELLBECOMING LEDGER writer
  (ADR-2606082102, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the inochi/asobi/hokorobi/tsugite/tate/ugachi/busshi family —
  the datomic-isomorphic canonical-state layer shiori previously only EMITTED
  (datom_emit) but never PERSISTED).

  Persists shiori's GROUND datoms — the cohort / detractor / driver / mitigator NODES
  and the detraction/relief :en/* 縁 (AGGREGATE data, G1) — to a local append-only EDN
  log of content-addressed transactions. DERIVED state (wellbecoming-burden / relief /
  relief-gap, the integral of incident 縁 computed on READ, N1/G2) is NEVER stored.
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; shiori 栞 — WELLBECOMING LEDGER (append-only, content-addressed EAVT "
                        "commit-DAG of the cohort/detractor/driver/mitigator graph). Generated; DO NOT "
                        "hand-edit. A RELIEF map at COHORT-AGGREGATE scale, never per-person affect. "
                        "ADR-2606082102 / ADR-2605312345.\n")))
