(ns shiori.methods.datom-emit
  "shiori 栞 — kotoba Datom-log emitter (canonical EAVT state, ADR-2605312345).
  1:1 Clojure port of `methods/datom_emit.py` (ADR-2606082100).

  Projects the wellbecoming-detraction graph into append-only kotoba Datoms [e a v tx op] — the
  first-class canonical state (NOT a projection cache). Two strata:

    GROUND (durable, op :add) — one datom per (entity, attribute, value): the cohort / detractor /
      driver / mitigator nodes and the :en/* 縁. This IS the Datom log.

    DERIVED (transient, :bond/is-transient true) — the edge-primary burden / relief / relief-gap /
      imposed / route-coverage integrals. Per N1/G2 these are computed on READ and are NOT stored
  ...
  affect / happiness score, NO biometric; every :cohort carries :cohort/scope :aggregate and
  drivers are structural patterns. The emitted Datom text carries no per-person key.

  Reuses shiori.methods.analyze (load-graph / read-edn / analyze). House style: Python ':…'
  ...
  shiori.methods.analyze/load-graph returns an array-map (order lost past 8 keys → 39-node seed),
...
(defn- order-of
  ...
  accumulation maps), else (keys). The keyword is qualified to shiori.methods.analyze."
  [d]
  (or (:shiori.methods.analyze/order (meta d)) (keys d)))
...
(defn emit
  ...
  ([nodes edges res tx]
   (let [L (transient [])]
     (conj! L ";; shiori 栞 — GENERATED kotoba Datom log (ADR-2606082100). DO NOT hand-edit.")
     ...
     (conj! L ";; G1: cohort-aggregate / structural only — NO individual records / affect score / biometric.")
     (conj! L "[")
     ...
     (doseq [nid (ranked-items (get res "burden"))]
       (conj! L (str "[" (fmt nid) " :bond/wellbecoming-burden " (fmt-g (get-in res ["burden" nid]))
                     " " tx " :derived] ;; :bond/is-transient true")))
     (doseq [nid (ranked-items (get res "relief"))]
       (conj! L (str "[" (fmt nid) " :bond/relief-buffer " (fmt-g (get-in res ["relief" nid]))
                     " " tx " :derived] ;; :bond/is-transient true")))
     (doseq [nid (ranked-items (get res "gap"))]
       (conj! L (str "[" (fmt nid) " :bond/relief-gap " (fmt-g (get-in res ["gap" nid]))
                     " " tx " :derived] ;; :bond/is-transient true")))
     (doseq [nid (ranked-items (get res "imposed"))]
       (conj! L (str "[" (fmt nid) " :bond/detraction-imposed " (fmt-g (get-in res ["imposed" nid]))
                     " " tx " :derived] ;; :bond/is-transient true")))
     (doseq [nid (ranked-items (get res "route_coverage"))]
       (conj! L (str "[" (fmt nid) " :bond/route-coverage " (fmt-g (get-in res ["route_coverage" nid]))
                     " " tx " :derived] ;; :bond/is-transient true")))
...
#?(:clj
   (defn load-file*
     "Read + parse a wellbecoming EDN graph file → {:nodes :edges} with ::node-order metadata."
     [path]
...
#?(:clj
   (defn -main
     ...
     (let [argv (vec argv)
           ...
           out (io/file outdir "wellbecoming-datoms.kotoba.edn")]
       (.mkdirs outdir)
       (spit out (emit nodes edges res tx))
       (println (str "shiori datom log → " out " (" (count nodes) " nodes + " (count edges)
                     " 縁, tx=" tx ")"))
       0)))
